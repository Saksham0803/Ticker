    ...
    if current_user.is_admin:
        if request.method == 'POST':
            ...
            if len(title_m) >= 1:   # and len(starring_m) >= 1 and len(production_m)
                ...
                new_movie = Movie(title=title_m, poster=poster_m, times_watched=no_watched_movie, movie_admin_id=current_user.id)   #   starring=starring_m, production_house=production_m
    ...
    if current_user.is_admin:
        ...
        if request.method == 'POST':
            ...
            if t_in >= 1 and available_seats >= 1: #movie>=1
                new_s = Show(screened_m=screened_m, t_in=t_in, movie=movie, t=t.name,address_t=t.address, datetime=date_time, t_admin_id=current_user.id, available_seats=available_seats, cost=cost)   #theater_address_link=t.location
                print(new_s)
